...
/*! 
    ...
    @returns {@link Point} object that has the x and y coordinets set. If the z coordinate is 0 it means the point is not touched. If z is 1, it is currently touched.
*/
/**************************************************************************/
Point Adafruit_FT6206::getPoint(uint8_t n) {
  readData();
  if ((touches == 0) || (n > 1)) {
	//Serial.println("No touches detected");
    return Point(0, 0, 0);
  } else {
	//Serial.print("Touches detected: ");Serial.println(touches);
