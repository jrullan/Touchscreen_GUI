...
SeeedstudioTouch::SeeedstudioTouch(uint8_t xp, uint8_t yp, uint8_t xm, uint8_t ym) {
    _yp = yp;
    _xm = xm;
    _ym = ym;
    _xp = xp;
...
Point SeeedstudioTouch::getPoint(void) {
    int x, y, z = 1;
...
#else //defined (__AVR__) || defined(TEENSYDUINO)
...
bool SeeedstudioTouch::isTouching(void)
{
    Point p = getPoint();
...
uint8_t SeeedstudioTouch::touched(void){
	return isTouching();
}